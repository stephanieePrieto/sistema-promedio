    ...
    try {
        ...
        if (!estudiante) {
            return res.status(404).send(
                ...
                <div class="acciones">
                    <a class="btn btn-secundario" href="/alumnos">Volver al listado</a>
                </div>
                `)
            );
        }
        ...
        const filasMaterias = estudiante.materias
            ...
            <div class="fila-resultado">
                ...
                <div>${m.creditos}</div>
                <div>${m.calificacion !== null && m.calificacion !== undefined
        ...
        <div class="datos-alumno">
            ...
            <div>${escaparHTML(estudiante.matricula)}</div>
            </div>
            <div>
            <label>Nombre</label>
            <div>${escaparHTML(estudiante.nombre)}</div>
            </div>
        </div>
        ...
        res
            .status(500)
            .send(paginaBase('Error', '<h1>Ocurrió un error al generar la boleta.</h1>'));
    }
});
